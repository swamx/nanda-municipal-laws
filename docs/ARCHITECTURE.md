...
Expose real NYC municipal law text through a small, stateless REST API that an autonomous agent can call: search for relevant sections, look one up directly, resolve its cross-references, filter by penalty/permit relevance, then retrieve the full document/chunk for citation. Positioned as a "Municipal Legal Intelligence Service" — deterministic, citation-backed, explainable — rather than a RAG/LLM-answer system: no embeddings, no vector database, no LLM calls, no server-synthesized "answer." The API returns structured results plus a mechanical `reasoning` string explaining *how* a result was derived; the calling agent (see `SKILL.md`) composes the final natural-language answer, sources, and confidence from those citations.
...
- **`mentions_penalty`** / **`mentions_permit`**: fixed keyword lists (`penalty`/`fine`/`violation`/`unlawful`/... and `permit`/`license`/`authorized`/...), substring-matched. Broad by design (`mentions_permit` includes "authorized," so it flags exception clauses too, not only affirmative permit-application sections) — a useful filter, not a legal determination. See [DATA_SOURCE.md](./DATA_SOURCE.md) for the full caveat.
...
**No separate migration/backfill tool for v2 fields**: `pipeline.ingest_url` already upserts documents and fully replaces chunks on every re-ingestion. Once `pipeline.py` computes the v2 fields, re-running `scripts/seed_admin_code.py` regenerates every existing Title 24 chunk with the new metadata for free, via the same idempotent mechanism.

## Search: in-app scoring, not MongoDB `$text`

MongoDB's native `$text`/`textScore` search requires a server-side text index — which, per the above, this Atlas role cannot create. `app/search_scoring.py` implements a small TF-style scorer instead. `app/retrieval.py::search_chunks()` is the shared retrieval function (equality-filter the candidates, score in Python, build a `reasoning` string) used by `/search`, `/penalties`, and `/permits` alike, so the retrieval logic lives in one place rather than being duplicated per endpoint. Filters now include `document_type`/`agency`/`topic` alongside the original `title_num`/`chapter_num`, plus `mentions_penalty`/`mentions_permit` for the topic-filter endpoints.

This is a keyword search, not a semantic one — term frequency drives ranking, not phrase meaning. Documented as a real, user-visible limitation in [DATA_SOURCE.md](./DATA_SOURCE.md) and `SKILL.md`, not hidden: e.g. a bare "noise" query ranks § 24-220 ("Noise mitigation plan") above § 24-222 ("After hours and weekend limits on construction work"), since 24-222's body text never uses the word "noise" at all.

Fine at this scale (currently ~150 chunks across both sources); if coverage grows to the point where fetching all candidates into Python becomes a bottleneck, the fix is either getting `createIndex` granted on `dl-laws` (switch back to native `$text`) or moving to Atlas Search (Lucene-based, doesn't need the classic `createIndex` privilege).
...
Vercel serverless functions don't share process memory across invocations or cold starts, so a naive in-process rate limiter (a `dict` of counters) would not enforce a real per-client limit once traffic spans more than one warm instance. `app/rate_limit.py` implements two FastAPI dependencies backed by the same mechanism: `rate_limiter` (applied to `search`, `sections`, `penalties`, `permits`, `documents`, and `health` routers, default `RATE_LIMIT_PER_MINUTE` = 10) and `ingest_rate_limiter` (applied only to the `ingest` router, default `INGEST_RATE_LIMIT_PER_MINUTE` = 1 — much stricter, since that endpoint triggers outbound fetches and Atlas writes rather than just reads). Both call a shared `_check_rate_limit(request, db, scope, limit)` helper that increments a per-minute counter document in `dl-laws` (`type: "ratelimit"`, keyed by `scope` + `client_id` + `window_start`) via an atomic `find_one_and_update` with `$inc`, and rejects with `429` once the count for that scope exceeds its limit in the current 60-second window. The `scope` field keeps the two limiters' counters independent, so a client hitting `/ingest` once doesn't eat into their `/search` budget or vice versa. Client identity is best-effort: the first IP in `X-Forwarded-For` (Vercel's proxy header), falling back to the raw socket address.
...
app/
  ...
  routers/               search, sections (+/related), penalties, permits, documents, ingest, health/version
  ingestion/
