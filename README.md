...
| [docs/presentation/](./docs/presentation/) | 14-slide pitch deck as `.pptx`, `.pdf`, and an editable `DECK_CONTENT.md` — ordered around three story vignettes first, then architecture, corpus scale, engineering highlights, a "why not an LLM?" comparison, and a single condensed API capability table (`/health`/`/version`/`/pubkey`/`/ingest` kept to a footnote, not their own slide) |
