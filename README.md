...
| [docs/presentation/](./docs/presentation/) | 16-slide pitch deck as both `.pptx` and `.pdf` — the problem, the capability pitch, architecture, corpus stats, the full API list (agent-facing endpoints kept separate from `/health`/`/version`/`/pubkey`/`/ingest`), three story vignettes, and engineering highlights |
